	...
	**/
		...
		$.post('/api/' + call,{
			...
			function(data) {
				...
				$.each(json, function (index, value) {
					d.push([parseInt(index),parseInt(value)]);
					//console.log(d);
				});
				flots_chart_cb(d, div)
			}
		);
	...
	function setup_flots() {
		...
		/* Call login chart */
		flots_chart('reports.users.by.day', 'totalUsers');
		flots_chart('reports.logins.by.day', 'userLogins');
	...
	function setup_quick_stats() {

		$.post('/api/reports.quick.stats',{
			},
			function(data) {
				var json = $.parseJSON(data);
	...
	function setup_user_types()
	{
		if ($('#userTypes').length){ 
			$.post('/api/reports.user.types',{
				},
				function(data) {
					var json = $.parseJSON(data);
					var d = [];
					$.each(json, function (index) {
						d[index] = {
						   ...
						   value: parseInt(json[index]["value"])
						};
						//console.log(d);
					});
